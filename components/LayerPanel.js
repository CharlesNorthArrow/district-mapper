...
export const NATIONAL_LAYERS = ['us-senate', 'tribal-lands', 'urban-areas'];
            ...
            <div style={styles.sectionBody}>
              {dataBatches.map((batch) => (
                <div key={batch.id} style={{ ...styles.layerRow, justifyContent: 'space-between' }}>
                  ...
                  {(() => {
                    const isClustered = clusteredBatchIds.has(batch.id);
                    return (
                      <button
                        onClick={() => onToggleBatchCluster?.(batch.id)}
                        title={isClustered
                          ? `Showing ${batch.label} as clusters — click to show individual points`
                          : `Show ${batch.label} as clusters (groups nearby points)`}
                        style={{
                          background: 'none', border: 'none', cursor: 'pointer',
                          fontSize: 13, padding: '0 0 0 6px', lineHeight: 1, flexShrink: 0,
                          color: isClustered ? 'var(--mid-blue)' : '#7a8fa6',
                        }}
                      >⦿</button>
                    );
                  })()}
