...
export async function assignDistricts(points, layerGeojson, onProgress, customMeta = {}) {
  ...
  const layerEntries = Object.entries(layerGeojson).map(([layerId, geojson]) => {
    ...
    const field = config?.districtField ?? cityConfig?.districtField ?? customMeta[layerId]?.nameField;
