...
export const LAYER_CONFIG = {
  counties: {
    ...
    // 0.003 (~333m) was too coarse — NY coastal/river counties were mislabelling
    // points near borders. 0.0001 (~11m) keeps NY ~1–3 MB and Yates jumps from
    // 24 → ~95 vertices, Suffolk gains the Long Island coastline detail.
  ...
  'county-subdivisions': {
    ...
    districtField: 'NAME',
    stateField: 'STATE',
    maxAllowableOffset: 0.0001,   // ~11m — standard precision across all layers
  ...
  zcta: {
    ...
    districtField: 'ZCTA5',
    stateField: null,
    // 0.001 (~110m) still mismatched points near ZCTA edges in dense urban
    // areas. 0.0001 (~11m) requires streaming the response (see boundaries.js)
    // because some states exceed the Vercel 4.5MB function response cap.
    maxAllowableOffset: 0.0001,
    resultRecordCount: 2000,
  },
  congressional: {
    ...
    queryMode: 'byState',          // ~26 NY districts beats fetching all 444 nationwide
    districtField: 'NAME',
    stateField: 'STATE',           // 2-digit FIPS string, e.g. '17' for Illinois
    maxAllowableOffset: 0.0001,
    resultRecordCount: 60,         // max districts in any state: 52 (California)
  },
  'us-senate': {
  ...
  'school-unified': {
    ...
    // 0.002 (~220m) was misassigning points along irregular district edges.
    // 0.0001 (~11m) matches the counties/zcta precision and requires the
    // streaming response in boundaries.js to stay under the function cap.
    maxAllowableOffset: 0.0001,
