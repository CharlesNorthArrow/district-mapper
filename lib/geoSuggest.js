...
// Maps each Tier 1 city slug to its US state — used so city points are attributed to the
// correct state rather than relying on overlapping state bounding boxes (e.g. NJ/NY near NYC)
const CITY_TO_STATE = {
  'nyc':          'New York',
  'la':           'California',
  'chicago':      'Illinois',
...
export function suggestGeographies(points) {
  ...
  for (const { lng, lat } of points) {
    ...
    // City detection — checked first so city points are attributed to the correct state
    // instead of an overlapping state bbox (e.g. NJ bbox covers Manhattan)
    let cityState = null;
    for (const [slug, bbox] of Object.entries(CITY_BBOX)) {
      if (inBbox(lng, lat, bbox)) {
        cityHits.add(slug);
        if (!cityState) cityState = CITY_TO_STATE[slug];
      }
    }

    if (cityState) {
      stateCounts[cityState] = (stateCounts[cityState] || 0) + 1;
    } else {
      // State detection — only for points not inside a known city bbox
